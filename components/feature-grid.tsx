...
export function FeatureGrid({ locale }: FeatureGridProps) {
  ...
  return (
      <div >
    <FeatureGridAnimated locale={locale} ref={wrapperRef as React.RefObject<HTMLDivElement>}>
        {featureKeys.map((key, i) => {
          const Icon = featureIcons[i]
          const href = featureHrefs[i]
          
          return (
            <Card key={key} className={cn("group relative h-full bg-card border-border shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 py-2 gap-0 card rounded-md"
            )}>
