...
import { PlayerLandingClient } from "./player-landing-client";

export async function PlayerLandingServer() {
        ...
        <div className="container relative mx-auto px-4 py-16 md:py-24">
          <div className="mx-auto max-w-4xl text-center">
            <h1 className={`text-balance text-4xl md:text-6xl font-bold tracking-tight text-foreground mb-6 ${locale === 'ar' ? 'font-arabic-heading' : 'font-mono tracking-wider'}`}>
              {locale === 'ar' ? 'منصة اللاعبين' : 'Player Platform'}
            </h1>
            <p className={`text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8 ${locale === 'ar' ? 'font-arabic-body' : ''}`}>
              {locale === 'ar' ? 'انضم إلى مجتمع رياضي عالمي واستمتع بتجربة رياضية شاملة' : 'Join a global sports community and enjoy a comprehensive sports experience'}
            </p>
          </div>
        </div>
      </section>

      {/* Player Landing Content */}
      <section className="container mx-auto px-4 py-16">
        <div className="mx-auto max-w-7xl">
          <PlayerLandingClient locale={locale} />
        </div>
      </section>
    </div>
  );
