...
import { PlayerRegisterClient } from "./player-register-client";

export async function PlayerRegisterServer() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        ...
        <div className="container relative mx-auto px-4 py-16 md:py-24">
          <div className="mx-auto max-w-4xl text-center">
            <h1 className={`text-balance text-4xl md:text-6xl font-bold tracking-tight text-foreground mb-6 ${locale === 'ar' ? 'font-arabic-heading' : 'font-mono tracking-wider'}`}>
              {locale === 'ar' ? 'انضم إلى منصة فيكرو الرياضية' : 'Join Ficro Sports Platform'}
            </h1>
            <p className={`text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8 ${locale === 'ar' ? 'font-arabic-body' : ''}`}>
              {locale === 'ar' ? 'ابدأ رحلتك الرياضية مع آلاف اللاعبين حول العالم' : 'Start your sports journey with thousands of players worldwide'}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="mx-auto max-w-2xl">
          <PlayerRegisterClient locale={locale} />
        </div>
      </section>
    </div>
  );
